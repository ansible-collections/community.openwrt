...
def test_find_module_util_script_raises_when_uc_missing(action):
    with patch.object(Path, "exists", return_value=False):
        with pytest.raises(UcodeModuleNotFound) as exc_info:
            action._find_module_util_script("_ansible_common")
    assert "_ansible_common" in str(exc_info.value)
    assert "_ansible_common.uc" in str(exc_info.value)


def test_find_module_util_script_returns_path_under_module_utils(action):
    with patch.object(Path, "exists", return_value=True):
        result = action._find_module_util_script("_ansible_common")
    assert str(result).endswith("_ansible_common.uc")
    assert "module_utils" in str(result)


def test_transfer_module_utils_transfers_declared_utils(action):
    action.module_utils = ["_ansible_common"]
    ...
    assert remote_util.endswith("_ansible_common.uc")
