...
class UcodeOpenwrtActionBase(ActionBase):
    ...
    (``plugins/module_utils/_ansible_common.uc``) via a relative ``import``.

    This action plugin:
      1. Transfers the module ``.uc`` file and its ``_ansible_common.uc`` helper
    ...
    def _run_ucode_module(self, module_name, module_args, task_vars):
        ...
        try:
            ...
            cmd = f"ucode {shlex.quote(remote_module)} {shlex.quote(args_path)}"
            ...
            if not stdout.strip():
                ...
                try:
                    # The module prints a leading newline (mirroring Python's
                    # _return_formatted) to delimit the result block; strip it.
            ...
            # Never leave the module, helper and cleartext args behind on the
            # device (flash storage); honour _ansible_keep_remote_files. Only
            # clean up when the base class marked the tmpdir for removal (the
            # unit tests mock _make_tmp_path and never set that flag).
            if getattr(self, "_cleanup_remote_tmp", False):
                self._remove_tmp_path(tmp_dir)
